    ...
    def _handle_mouse_motion(self, x: int, y: int) -> None:
        ...
        if not self.game_manager.dev_mode:
            ...
            if self.dragging and isinstance(self.active, Slider):
                if self.graphics_manager.menu_open:
                    self.handle_drag(
                        x - self.game_manager.menu_margins[0], 
                        y - self.game_manager.menu_margins[1]
                    )
                else:
    ...
    def handle_click(self) -> None:
        ...
        if self.graphics_manager.menu_open:
            if pygame.rect.Rect.collidepoint(
                pygame.Rect(
                    self.active.rect.x + self.game_manager.menu_margins[0], 
                    self.active.rect.y + self.game_manager.menu_margins[1], 
                    self.active.rect.w, 
                    self.active.rect.h
                ), 
                (x, y)
