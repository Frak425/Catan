...
def create_buttons() -> object:
    ...
    play_b_w = 200
    play_b_h = 75
    play_button = Button("main_menu", (0, 100, 0), "PLAY", [screen_w / 2 - play_b_w / 2, screen_h / 2 - play_b_h / 1.75, play_b_w, play_b_h], "play", screen, game_manager.game_font, (0, 0))
    quit_button = Button("main_menu", (100, 0, 0), "QUIT", [screen_w / 2 - play_b_w / 2, screen_h / 2 + play_b_h / 1.75, play_b_w, play_b_h], "quit", screen, game_manager.game_font, (0, 0))
    title_screen_buttons = [play_button, quit_button]


    #create game setup buttons
    game_setup_back_button = Button("game_setup", (100, 0, 0), "Back", [screen_w / 4 - play_b_w / 2, screen_h / 2, 200, 100], "game_setup_back", screen, game_manager.game_font, (0, 0))
    gsb_w = 150
    gsb_h = 50
    game_start_button = Button("game_setup", (100, 0, 0), "Start", [screen_w / 4 * 3 - gsb_w / 2, screen_h / 8 * 6, gsb_w, gsb_h], "game_start", screen, game_manager.game_font, (0, 0))
    pnidb_size = screen_h / 20
    player_num_increase_button = Button("game_setup", (0, 100, 0), "+", [screen_w / 4 - pnidb_size / 2 + 100, screen_h / 4 * 2.5, pnidb_size, pnidb_size], "player_num_increase", screen, game_manager.game_font, (0, 0))
    player_num_decrease_button = Button("game_setup", (0, 100, 0), "-", [screen_w / 4 - pnidb_size / 2 - 100, screen_h / 4 * 2.5, pnidb_size, pnidb_size], "player_num_decrease", screen, game_manager.game_font, (0, 0))
    player_choose_color_cycle_button = Button("game_setup", (0, 0, 0), "->", [10, 10, 10, 10], "player_choose_color_cycle", screen, game_manager.game_font, (0, 0))
    difficulty_level_easy_button = Button("game_setup", (0, 0, 0), "easy", [10, 10, 10, 10], "difficulty_level_easy", screen, game_manager.game_font, (0, 0))
    difficulty_level_medium_button = Button("game_setup", (0, 0, 0), "medium", [10, 10, 10, 10], "difficulty_level_medium", screen, game_manager.game_font, (0, 0))
    difficulty_level_hard_button = Button("game_setup", (0, 0, 0), "hard", [10, 10, 10, 10], "difficulty_level_hard", screen, game_manager.game_font, (0, 0))
    sob_size = screen_w / 12 / 1.5
    sob_offset = screen_h / 24 / 1.5
    settings_menu_button = Button("game_setup", (100, 0, 0), "image", [screen_w - sob_offset - sob_size, sob_offset, sob_size, sob_size], "settings_menu", screen, game_manager.game_font, (0, 0))
    ...
    board_buy_settlement_button = Button("board", (0, 0, 0), "image", [10, 10, 10, 10], "board_buy_settlement", screen, game_manager.game_font, (0, 0))
    board_buy_city_button = Button("board", (0, 0, 0), "image", [10, 10, 10, 10], "board_buy_city", screen, game_manager.game_font, (0, 0))
    board_buy_road_button = Button("board", (0, 0, 0), "image", [10, 10, 10, 10], "board_buy_road", screen, game_manager.game_font, (0, 0))
    board_buy_development_button = Button("board", (0, 0, 0), "image", [10, 10, 10, 10], "board_buy_development", screen, game_manager.game_font, (0, 0))
    board_roll_dice_button = Button("board", (0, 0, 0), "image", [10, 10, 10, 10], "board_roll_dice", screen, game_manager.game_font, (0, 0))
    settings_menu_button = Button("game_setup", (100, 0, 0), "image", [screen_w - sob_offset - sob_size, sob_offset, sob_size, sob_size], "settings_menu", screen, game_manager.game_font, (0, 0))
...
def create_menu() -> Menu:
    ...
    menu = Menu(screen, game_manager.game_font, "static", menu_size, init_location, final_location, bckg_color=background_color)
    return menu

def init() -> None:
    for i in range(game_manager.players_num):
        game_manager.players_list.append(Player(game_manager.player_colors, game_manager.points_to_win))

    global board
    board = Board(screen_w / 27.32, screen_h, screen_w, game_manager.num_tiles, screen, game_manager.game_font, game_manager.font_size)
    board.assign_tile_locations()
    board.assign_tiles()
    board.assign_tile_classes()

def check_game_over() -> bool:
    for i in range(len(game_manager.players_list)):
        if game_manager.players_list[i].score >= game_manager.points_to_win:
            return True
    return False
...
while running:
    ...
    elif (game_manager.game_state == "init"):
        init()
        game_manager.game_state = "game_ongoing"
        continue
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if (game_manager.game_state == "main_menu"):
                ...
                if (type(button_clicked) != bool): #if a button is clicked, find which one and act accordingly
                    ...
                    if (button_name == "play"):
                        game_manager.game_state = "game_setup"
